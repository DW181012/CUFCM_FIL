...
// #include "../../globals.hpp"
  ...
  void equal_area_seeding_centric(Real *const pos_ref, Real *const polar_dir_refs, Real *const azi_dir_refs, Real *const normal_refs, const int N, shape_fourier_description& shape, Real disc_r, Real singularity){
    ...
    for (int n = 0; n < N; n++){
      ...
      //const Real phi = std:: atan2(X[3*n + 1], X[3*n]);
      //const Real phi = std:: atan2(X[3*n + 1], X[3*n]) + atan(1) * 4;
      //const Real phi = std:: atan2(X[3*n + 1], X[3*n]) + atan(1) * 4 * n;
      const Real phi = std:: atan2(X[3*n + 1], X[3*n])  + atan(1) * 4 * (n + 1);
    ...
    for (int i = 0; i < dim_true; i++){
      ...
      for (int j = 0; j < dim_true; j++){
        ...
        switch (CHOICE_OF_CONTOUR) {
          case 0:
            beating_orientation[0] = cos(angle);
            beating_orientation[1] = sin(angle);
            break;
          case 1:
            beating_orientation[0] = -cos(angle);
            beating_orientation[1] = -sin(angle);
          ...
          case 2:
            ...
            break;
          case 3:
            beating_orientation[0] = -sin(angle - angle_tilt);
            beating_orientation[1] = cos(angle - angle_tilt);
            break;
          case 4:
            beating_orientation[0] = -sin(angle + angle_tilt);
            beating_orientation[1] = cos(angle + angle_tilt);
            break;
          case 5:
            beating_orientation[0] = -sin(angle);
            beating_orientation[1] = cos(angle);
            break;
          case 6:
            beating_orientation[0] = -cos(angle/2);
            beating_orientation[1] = -sin(angle/2);
            break;
        }
  ...
  void mcc_seeding(Real *const pos_ref, Real *const polar_dir_refs, Real *const azi_dir_refs, Real *const normal_refs, const int N, shape_fourier_description& shape, Real disc_r){
  ...
  void defective_seeding(Real *const pos_ref, Real *const polar_dir_refs, Real *const azi_dir_refs, Real *const normal_refs, const int N, shape_fourier_description& shape, Real disc_r){
    ...
    for (int i = 0; i < dim_true; i++){
      ...
      for (int j = 0; j < dim_true; j++){
        ...
        if ((i < defect_start)||(i > defect_end)||(j < defect_start)||(j > defect_end)){
          //outFile << " " << 0;
          //outFile << " " << final_state[n+2];
          pos_ref[3*n] = i * spacing - length_side/2;
          pos_ref[3*n + 1] = j * spacing - length_side/2;
 ...
 void full_mcc_seeding(Real *const pos_ref, Real *const polar_dir_refs, Real *const azi_dir_refs, Real *const normal_refs, const int N, shape_fourier_description& shape, Real disc_r){
    
    // calculate the decomposition of cilia
    int M_cilia = 358;
    int N_patch = N / M_cilia;
    int N_patch_side = ceil(sqrt(N_patch));
    int N_patch_size = pow(N_patch_side,2);
    ...
    double length_column = 16;
    int Tol = 5;
    for (int i = 1; i <= sqrt(M_cilia/2); i = i + 1){
      if ((M_cilia - i - 1) % (2 * i + 1) == 0){

          column_tot = i;
          row_tot = (M_cilia - i - 1) / (2 * i + 1);
          if (abs(column_tot * length_column - row_tot * length_row) < Tol){
              break;
          }
      }
    ...
    int cilia_tot = row_tot * column_tot + (row_tot + 1) * (column_tot + 1);
    double phase[N];
    double patch_phase[N_patch];
    double patch_orientation[N_patch];
    double x_shift = (0.5 * column_tot + 1) *length_column;
    double y_shift = (0.5 * row_tot + 0.5) * length_row;
    double patch_spacing = 100;
    ...
    std::normal_distribution<double> dis3(0, 0.5);
    // Use dis to transform the random unsigned int generated by gen into a 
    // double in [0, 1). Each call to dis(gen) generates a new random double.
    ...
    for (int patch_index = 0; patch_index < N_patch; patch_index++){
      ...
      do {
        patch_orientation[patch_index] = dis3(gen3);
      } while (patch_orientation[patch_index] < -1 || patch_orientation[patch_index] > 1);
      ...
      for (int i = 0; i < 2 * row_tot + 1; i++){
        ...
        for (int j = 0; j < column; j++){
          ...
          pos_ref[3*n] = patch_center_x + (j + 1 + (column == column_tot) * 0.5) * length_column - x_shift;
          pos_ref[3*n + 1] = patch_center_y + (i + 1) * length_row * 0.5 - y_shift;
          pos_ref[3*n + 2] = 0;

          phase[n] = i * atan(1) * 8 / (2*row_tot) + patch_phase[patch_index];
          outFile << " " << phase[n];

          const Real theta = 0.001;
          const Real phi = patch_orientation[patch_index];
  ...
  void seed_filaments(Real *const filament_references, Real *const polar_dir_refs, Real *const azi_dir_refs, Real *const normal_refs){
    ...
    shape_fourier_description shape;
      ...
      std::cout << "multi-patch mcc seeding for a full simulation of particle transport" << std::endl;
      double disc_radius = FIL_SPACING * NFIL / 18;
      full_mcc_seeding(filament_references, polar_dir_refs, azi_dir_refs, normal_refs, NFIL, shape, disc_radius);
